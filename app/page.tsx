...
export default function Home() {
      ...
      for (let dayIdx = 0; dayIdx < (itinerary.days || []).length; dayIdx++) {
        ...
        acts.forEach((act: any, actIdx: number) => {
          ...
          if (!isNaN(lat) && !isNaN(lng) && lat !== 0 && lng !== 0) {
            ...
            // פופ-אפ עם כפתור לכרטיסים (אם יש) וכפתור ניווט שמבוסס על חיפוש השם
  ...
  return (
    <>
      ...
      `}} />
      <main className="min-h-screen bg-gradient-to-br from-slate-900 via-sky-950 to-indigo-950 py-12 px-4 sm:px-6 lg:px-8 text-gray-100" dir={language === 'he' ? 'rtl' : 'ltr'} style={{ fontFamily: "'Assistant', sans-serif" }}>
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-medium transition-all cursor-pointer ${
                          isSelected ? 'bg-sky-500 text-white shadow-lg font-bold border border-sky-300' : 'bg-white/5 text-sky-100 hover:bg-white/10 border border-white/15'
                        }`}
                      >
                        {style.label}
                      </button>
                ...
                {itinerary.hotelRecommendation && (
                  <div className="bg-slate-800/90 border border-sky-500/50 rounded-2xl p-6 mb-8 shadow-xl">
                    <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
                      🏨 {language === 'he' ? 'המלצת לינה' : 'Accommodation Recommendation'}
                    </h3>
