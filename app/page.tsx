  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-sky-950 to-indigo-950 py-12 px-4 sm:px-6 lg:px-8 text-gray-100" dir={language === 'he' ? 'rtl' : 'ltr'}>
      <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/20 text-white focus:ring-2 focus:ring-sky-400 focus:outline-none transition-all shadow-inner"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-sky-100 mb-2">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-slate-900 border border-white/20 text-white focus:ring-2 focus:ring-sky-400 focus:outline-none transition-all shadow-inner"
                >
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-medium transition-all cursor-pointer ${
                        isSelected
                          ? 'bg-sky-500 text-white shadow-lg scale-105 font-bold border border-sky-300'
                          : 'bg-white/5 text-sky-100 hover:bg-white/10 border border-white/15'
                      }`}
                    >
                      {style.label}
                    </button>
        ...
        {/* הצגת המסלול המלא והמפה */}
        {itinerary && (
          <div className="space-y-8">
            
            {/* מפה אינטראקטיבית מסוג לווין/גלובוס */}
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-4 shadow-2xl border border-white/20 overflow-hidden">
              <div className="flex items-center justify-between mb-3 px-2">
                ...
                <span className="text-xs text-sky-300 bg-sky-500/20 px-3 py-1 rounded-full border border-sky-400/30">
                  {language === 'he' ? 'לחץ על כל כרטיסייה למיקוד במפה 📍' : 'Click any card to focus on map 📍'}
                </span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/20">
              ...
              <div className="space-y-6">
                {(itinerary.days || itinerary.itinerary || []).map((day: any, index: number) => {
                  ...
                  return (
                    <div key={index} className="bg-white/5 rounded-2xl p-6 border border-white/15 shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        <span 
                          className="px-4 py-1.5 text-white font-bold rounded-full text-sm shadow-md"
                      ...
                      <div className="space-y-4">
                        {activities.length > 0 ? (
                          activities.map((act: any, actIdx: number) => {
                            const actName = act.name || act.title || act.placeName || act.activity || act.spotName || 'פעילות';
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {!isNaN(actLat) && !isNaN(actLng) && (
                                      <span 
                                        className="text-xs px-2.5 py-1 text-white rounded-full font-mono shadow-sm transition-all" 
                                        style={{ backgroundColor: dayColor }}
                                        dir="ltr"
                                      >
                                        📍 {actLat.toFixed(2)}, {actLng.toFixed(2)}
                                      </span>
                                    )}
                                  </div>
                                </div>
                                {actDesc && (
                                  <p className="text-sky-100/90 text-sm leading-relaxed" dir="auto">
                                    {actDesc}
                                  </p>
                                )}
                              </div>
